...
public class StringToolBox {
  ...
  public static String nameEcho(String s) {
	...
	/**
		 * Checks if the last 2 characters in a string are both stars.
		 * We just look at the substring of the last 2 chars. 
		 */
  ...
  public static String removeDashes(String s) {
	...
	/*
	 * public static String caesar(String s, int n) { //String alphabet =
	 * "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
	 * for (int i=1; i<s.length(); i++) { char charAt = ALPHABET.charAt(i);
	 * s=s.replace(charAt, ALPHABET.charAt(i+n+25)); } return s; }
	 */
	...
	public static boolean isPalindrome(String s) {
		...
		/**
		 * Converts mm/dd/yy date to dd-mm-yy.
		 * We look for the first and last indices of "/"
		 * to find the month, day, and year. The substrings between these
		 * are the m/d/y, so we take those and concatenate them in the desired output.
		 */
	public static String dateStr(String s) {
		 ...
		 * Done by replacing all 0's with a placeholder, 1's with 0, then 
		 * replacing the placeholder with 1.
		 */
	...
	public static Boolean containsSameChar(String s) {
		/*ALTERNATIVE METHOD
		 * String matchingChars[] = s.split(s.substring(0, 1) + "+"); return
		 * matchingChars.length == 0;
		 */
		/*
		 * Checks if all the chars in the string are the same.
		 * We replace every occurence of the 1st character of the string
		 * with an empty string. If all the string's chars are the same, this should 
	...
	/**
		 * Removes all text between begin-comment and end-comment signs.
		 * Loops through string, finds substrings that aren't in comments, and concatenates them.
		 */
  ...
  Converts lastname, firstname to firstname lastname format.
    */
	public static String convertName(String s) {
		...
		/**
		 * You first find the index of a character in the string to be encoded,
		 * then compare that to the index of that character in the alphabet. However,
		 * since you are shifting the alphabet basically, you add n to the alphabet
		 * index. So I put the alphabet, upper and lowercase, into a string: Then I make an
		 * empty string which will contain the encrypted string. Then, I have a loop
		 * which runs through all characters in the input; if it is a letter, the letter
		 * is encrypted by shifting it on the alphabet string and then adding it to the
	 ...
	 */
	 public static String caesar(String s, int n) { s = s.toLowerCase(); String
	 cipherText = "";
	 
	  for (int i = 0; i < s.length(); i++) { char x = s.charAt(i); if (x!=' '){ int
	  charPosition = ALPHABET.indexOf(s.charAt(i)); int keyVal = (n + charPosition)
	  % 26; char replaceVal = ALPHABET.charAt(keyVal); cipherText += replaceVal; }
	 } return cipherText; }
